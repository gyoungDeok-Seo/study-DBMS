...
/*2. join만 사용*/
select name, age, gender
from tbl_field_trip f
join tbl_apply a
on title like '%매미%' and f.id = field_trip_id
join tbl_child c
on a.child_id = c.id;

/*체험학습을 2개 이상 신청한 아이의 정보와 부모의 정보 모두 조회*/
/*join, 서브쿼리 함께 쓰기(내가 생각하는 정답)*/
select c.name, c.age, c.gender, p.name, p.age, p.gender, address, phone
from tbl_parent p
join tbl_child c
on c.id in
(
...
/*2개의 테이블에서 정보를 가져오기 때문에 서브쿼리만 사용해선 불가능하다*/

/*참가자(지원자) 수가 가장 적은 체험학습의 제목과 내용 조회*/
/*join만 사용해서도 가능하지만 2개 이상일 경우 문제 발생*/
select title, content, count(child_id) apply_count
from tbl_field_trip f
join tbl_apply a
on f.id = a.field_trip_id
group by f.id
order by apply_count
limit 1;

/*참가자 수 최소값에 해당하는 것이 여러개 일 경우 대비(내가 생각하는 정답)*/
select title, content, count(child_id) from tbl_field_trip f
join tbl_apply a on f.id = a.field_trip_id
group by f.id
having count(child_id) =
(
    select min(ac.aplly_count)
    from
    (
        select count(child_id) aplly_count
        from tbl_apply
...
(
    ...
    (
        select min(ac.apply_count) from
        (
            select count(child_id) apply_count
            from tbl_apply
            group by field_trip_id
        ) ac
    )
...
/*서브 쿼리 최소화(내가 생각하는 정답)*/
select title, content, count(child_id)
from tbl_field_trip f
left outer join tbl_apply a
on f.id = a.field_trip_id
group by f.id
having count(child_id) <
(
    select floor(avg(aplly_count)) from
    (
        select count(child_id) aplly_count
        from tbl_apply
        group by field_trip_id
...
select * from tbl_apply;
delete from tbl_apply where id = 12;
...
(
    ...
    having count(child_id) <
    (
        select round(avg(aplly_count), 0)
        from
        (
            select count(child_id) aplly_count
            from tbl_apply
