...
VALUES(SEQ_PARENT.NEXTVAL,'한동석', '40', '경기도 남양주시', '01012341234', '남자');
...
SELECT CEIL(AVG(FIELD_TRIP_NUMBER)) FROM TBL_FIELD_TRIP;/*주로 많이 사용됨*/
 ...
 * ORDER BY: 정렬
 * 
 * ORDER BY [컬럼명] ASC : 오름차순인데, 생략가능 - 낮은 수부터
 * ORDER BY [컬럼명] DESC: 내림차순, 생략불가 - 높은 수부터
 * 
 * */

SELECT * FROM TBL_FIELD_TRIP
/*ORDER BY FIELD_TRIP_NUMBER ;*/
 ...
 * FROM절 : IN LINE VIEW
 * SELECT절 : CSALAR
 * WHERE절 : SUB QUERY - 값이 여러개이기에 IN으로 묶을 것
 * 
 * */
...
/*SELECT에서 *을 다른것과 같이 쓰고 싶을 때 - 소속 테이블명.*, 다른것 - 이런 형식으로 쓴다 */
/*집계 함수는 결과가 1개이기 때문에 SELECT절에는 쓸 수 없기에 서브 쿼리로 만들어 넣어준다*/
...
VALUES(SEQ_REPLY.NEXTVAL, '댓글 테스트1', 21, 3);
INSERT INTO TBL_REPLY
VALUES(SEQ_REPLY.NEXTVAL, '댓글 테스트2', 22, 1);
INSERT INTO TBL_REPLY
VALUES(SEQ_REPLY.NEXTVAL, '댓글 테스트3', 22, 2);
INSERT INTO TBL_REPLY
VALUES(SEQ_REPLY.NEXTVAL, '댓글 테스트4', 21, 2);

SELECT R.*, U.USER_ID FROM TBL_REPLY R;
